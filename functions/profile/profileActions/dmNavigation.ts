...
async function navigateToProfileViaSearch(
  ...
  if (directInput) {
    ...
    searchClicked = true;
    getLogger().info('ACTION', 'Found and clicked search input directly');
  }
  ...
  if (!searchClicked) {
    ...
    if (searchIcon) {
      ...
      if (searchInput) {
        ...
        searchClicked = true;
        getLogger().info('ACTION', 'Opened search panel and clicked input');
      }
    }
  }
  ...
  if (!searchClicked) {
    getLogger().info('ACTION', 'Search icon not found, trying explore page');
