  ...
  try {
    ...
    const statsData = await page.evaluate(() => {
      ...
      if (header) {
        ...
        const followersMatch = allText.match(/([\d.]+[KMB]?)\s*(followers?|follower)/i);
        ...
        return {
          followers: parseCount(followersMatch) || getCount(followersLink as Element),
          following: parseCount(followingMatch) || getCount(followingLink as Element),
