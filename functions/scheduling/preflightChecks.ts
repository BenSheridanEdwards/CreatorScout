...
export function checkDisplay(): { ok: boolean; message: string } {
  ...
  return {
    ok: false,
    message: 'No DISPLAY environment variable set. Start VNC or set DISPLAY=:0',
  };
}
 ...
 */
export async function getCompletedSessionsToday(profileId: string): Promise<{
...
export async function runPreflightChecks(
  ...
  if (ready) {
    ...
    logger.warn(
      'PREFLIGHT',
      `✗ Pre-flight checks failed: ${failures.join('; ')}`,
    );
  }
...
export async function filterCatchUpSessions(
  ...
  // Strict session time windows - sessions ONLY run within these hours
  const SESSION_WINDOWS = {
    morning: { start: 8, end: 10 }, // 8-10 AM only
    afternoon: { start: 14, end: 16 }, // 2-4 PM only
    evening: { start: 19, end: 21 }, // 7-9 PM only
  } as const;

  // Determine which sessions are within their designated time window
  const sensibleSessions: SessionType[] = [];

  for (const type of remaining) {
    const window = SESSION_WINDOWS[type];
    // Only allow session if we're currently within its time window
    if (currentHour >= window.start && currentHour < window.end) {
      sensibleSessions.push(type);
      logger.info(
        'PREFLIGHT',
        `${type} session can run (within ${window.start}:00-${window.end}:00 window)`,
      );
    } else {
      logger.info(
        'PREFLIGHT',
        `${type} session skipped - outside window (${window.start}:00-${window.end}:00), current hour: ${currentHour}`,
      );
    }
  }
