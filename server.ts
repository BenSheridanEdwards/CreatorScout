...
async function handleApi(
  ...
  if (req.method === 'GET' && url.pathname === '/api/health/detailed') {
    try {
      const { checkHealth, getRecentAlerts } =
        await import('./functions/shared/health/healthMonitor.ts');
  ...
  if (req.method === 'GET' && url.pathname === '/api/scheduler/status') {
    try {
      const { getNodeScheduler } =
        await import('./functions/scheduling/nodeScheduler.ts');
  ...
  if (req.method === 'GET' && url.pathname === '/api/proxy/usage') {
    try {
      const { getTodayProxyUsage, estimateMonthlyProxyCost } =
        await import('./functions/shared/proxy/proxyOptimizer.ts');
  ...
  if (req.method === 'GET' && url.pathname === '/api/data-quality') {
    try {
      const { checkDataQuality } =
        await import('./functions/shared/database/dataQualityMonitor.ts');
          ...
          try {
            ...
            if (metrics.length > 0) {
              ...
              return {
                ...run,
                profilesProcessed:
                  metric.profilesVisited || run.profilesProcessed || 0,
                creatorsFound: metric.creatorsFound || run.creatorsFound || 0,
                dmsSent: metric.dmsSent || run.dmsSent || 0,
                profilesChecked:
                  metric.profilesVisited || run.profilesChecked || 0,
              };
            }
          } catch {
  ...
  if (url.pathname === '/api/schedule/cron' && req.method === 'GET') {
    try {
      const { parseCrontab } =
        await import('./functions/shared/runs/crontabParser.ts');
  ...
  // GET /api/schedule - Return combined scheduled runs (scheduler + cron + config)
  if (url.pathname === '/api/schedule' && req.method === 'GET') {
    try {
      // Get scheduler jobs (primary source of truth)
      ...
      try {
        ...
        // Format scheduler jobs to match ScheduledRun interface
        schedulerJobs = todayJobs
          .filter((job: any) => job.status === 'pending')
          .map((job: any) => {
            // Format account name from kebab-case to Title Case
            const formattedAccountName = job.profileId
              .split(/[-_]/)
              .map(
                (word: string) =>
            ...
            return {
              ...
              name: `${job.sessionType.charAt(0).toUpperCase() + job.sessionType.slice(1)} Discovery`,
              source: 'scheduler',
            };
          });
      } catch (schedulerError) {
        console.warn('Scheduler not available:', schedulerError);
      }
      ...
      // Try to load one-off runs from config.json
      let configRuns: any[] = [];
      try {
        const configPath = join(process.cwd(), 'schedule.config.json');
        if (existsSync(configPath)) {
          const configData = await readFile(configPath, 'utf-8');
          const config = JSON.parse(configData);
          configRuns = (config.oneOff || []).filter((run: any) => {
            // Only include future one-off runs
            return new Date(run.scheduledTime) > new Date();
          });
        }
      } catch {
      ...
      // Combine all sources, scheduler jobs take priority
      const allScheduled = [...schedulerJobs, ...cronRuns, ...configRuns];
...
server.listen(PORT, BIND_ADDRESS, () => {
  ...
  if (DASHBOARD_API_KEY) {
    console.log(
      `API server listening on http://${BIND_ADDRESS}:${PORT} (remote access enabled with API key)`,
    );
  } else {
    console.log(
      `API server listening on http://${BIND_ADDRESS}:${PORT} (localhost only - set DASHBOARD_API_KEY for remote access)`,
    );
  }
  // eslint-disable-next-line no-console
